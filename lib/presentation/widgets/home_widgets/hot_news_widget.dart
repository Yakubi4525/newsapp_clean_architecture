...
import 'package:news_cleaan_arch_bloc/internal/dependencies/hot_news_bloc_module.dart';
...
class _HotNewsWidgetState extends State<HotNewsWidget> {
  GetHotNewsBloc _getHotNewsBloc;

  @override
  void initState() {
    _getHotNewsBloc = HotNewsModule.getBloc();
    _getHotNewsBloc.loadHotNewsModel();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: _getHotNewsBloc.subject.stream,
      builder:
          (BuildContext context, AsyncSnapshot<ArticleResponseModel> snapshot) {
        if (snapshot.hasData) {
          if (snapshot.data is HotNewsModelIntnState) {
            return Container();
          } else if (snapshot.data is HotNewsModelLoadingState) {
            return buildLoadingWidget();
          } else if (snapshot.data is HotNewsModelOkState) {
            return _buildHotNewsWidget(snapshot.data);
          } else if (snapshot.data is HotNewsModelErrorState) {
            return Center(
  ...
  Widget _buildHotNewsWidget(ArticleResponseModel data) {
