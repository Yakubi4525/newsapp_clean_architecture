...
import 'package:news_cleaan_arch_bloc/internal/dependencies/source_news_bloc_module.dart';
...
class _SourceDetailState extends State<SourceDetail> {
  ...
  GetSourceNewsBloc _getSourceNewsBloc;
  @override
  void initState() {
    _getSourceNewsBloc = SourceNewsModule.getBloc();
    _getSourceNewsBloc.loadSourceNewsModel(source.id);
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
              child: StreamBuilder(
                  stream: _getSourceNewsBloc.subject.stream,
                  builder: (BuildContext context,
                      AsyncSnapshot<ArticleResponseModel> snapshot) {
                    if (snapshot.hasData) {
                      if (snapshot.data is SourceNewsModelInitState) {
                        return Container();
                      } else if (snapshot.data is SourceNewsModelLoadingState) {
                        return buildLoadingWidget();
                      } else if (snapshot.data is SourceNewsModelOkState) {
                        return _buildSourceNewsWidget(snapshot.data);
                      } else if (snapshot.data is SourceNewsModelErrorState) {
                        return Center(
